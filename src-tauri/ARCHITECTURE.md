...
Lyrics retrieval first tries `/api/get` with the track's full metadata. If that returns no lyrics, only plain text, or HTTP 400, it searches by title and artist without album, including `&`/`and`/`+`/`plus` artist variants. A normalized title query is also tried when punctuation prevents any suitable match. Title and artist matching ignores punctuation differences, including apostrophe styles and commas. Candidates with matching durations (rounded to seconds) take priority, followed by durations less than three seconds apart; synced lyrics win ties. More distant results can supply plain text only, and a nearby synced result can replace an existing plain result only when its duration tier is at least as close. Read-only LRCLIB requests retry transient network, server, and response decoding failures up to three times. API error bodies support LRCLIB's `name`/`statusCode` fields.
