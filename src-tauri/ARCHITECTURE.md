...
Lyrics retrieval first tries `/api/get` with the track's full metadata. If that returns no lyrics, only plain text, or HTTP 400, it searches by title and artist both with the album and without it, including `&`/`and`/`+`/`plus` artist variants. The album query narrows LRCLIB's limited result set while still finding alternate editions. Parenthesized text can be stripped from either the local title or a search result, so titles such as `Feeling Good` and `Feeling Good (Live)` can match. A normalized title query is tried when punctuation prevents any suitable match. Title and artist matching ignores punctuation differences, including apostrophe styles and commas. Synced candidates with matching durations (rounded to seconds) take priority, followed by synced candidates less than five seconds apart; plain text is used only after both tiers fail. The original title is preferred over a stripped title within a tier. Read-only LRCLIB requests retry transient network, server, and response decoding failures up to three times, waiting at least two and then four seconds (or longer if `Retry-After` requests it, capped at 30 seconds). API error bodies support LRCLIB's `name`/`statusCode` fields.
